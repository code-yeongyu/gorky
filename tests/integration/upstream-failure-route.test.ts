...
describe("upstream failure route handling", () => {
  ...
  it("Given upstream still rejects after refresh When chat completions is called Then caller receives api error", async () => {
    // Given
    const apiKey = await createApiKey({
      name: "qa-key",
      allowedModels: ["grok-build"],
    ...
    const app = createApp({
      ...
      upstream: async () => Response.json({ error: { code: "unauthorized" } }, { status: 403 }),
      refreshClient: async (): Promise<TokenRefreshResult> => ({
        kind: "success",
    ...
    expect(store.apiKeys[0]?.lastUsedAt).toBeNull()
    expect(logText).toContain("proxy_request_failed")
    expect(logText).toContain("upstream_auth_failed")
    expect(logText).not.toContain(apiKey.plaintextKey)
