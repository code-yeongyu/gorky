...
describe("admin routes", () => {
  ...
  it("Given admin auth When revoking a key Then the key can no longer call the proxy", async () => {
    // Given
    ...
    // When
    const revokeResponse = await app.request(`/api/admin/keys/${keyId}/revoke`, {
      method: "POST",
      headers: {
        "x-admin-token": "dev-admin-token",
      },
    ...
    expect(revokeText).toContain(createdBody.keyPrefix)
    expect(revokeText).toContain('"revokedAt":1780000000000')
    expect(revokeText).not.toContain(createdBody.plaintextKey)
    expect(revokeText).not.toContain(store.apiKeys[0]?.keyHash)
    ...
    expect(logText).toContain("admin_key_revoked")
    expect(logText).not.toContain(createdBody.plaintextKey)
    expect(logText).not.toContain("SENSITIVE_REFRESH_SENTINEL")
  })
