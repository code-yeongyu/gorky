...
const dbPath = process.env["DB_PATH"] ?? "gorky.sqlite"

const adminToken = requiredEnv("ADMIN_TOKEN")
const encryptionSecret = requiredEnv("TOKEN_ENCRYPTION_SECRET")
const authIssuer = process.env["AUTH_ISSUER"] ?? "https://auth.x.ai"
const oidcClientId = process.env["OIDC_CLIENT_ID"] ?? "b1a00492-073a-47ea-816f-4c329264a828"
const grokClientVersion = process.env["GROK_CLIENT_VERSION"] ?? "0.2.16"
const cliProxyBaseUrl = process.env["GROK_CLI_PROXY_BASE_URL"] ?? "https://cli-chat-proxy.grok.com/v1"
const publicApiBaseUrl = process.env["GROK_PUBLIC_API_BASE_URL"] ?? "https://api.x.ai/v1"
const grokModelIds = process.env["GROK_MODEL_IDS"] ?? "grok-composer-2.5-fast,grok-build"
const qaMode = process.env["GORKY_QA_MODE"] === "true"

const refreshClient = createOAuthRefreshClient({ issuer: authIssuer, clientId: oidcClientId })
const authorizationClient = createOAuthAuthorizationClient({ issuer: authIssuer, clientId: oidcClientId })
